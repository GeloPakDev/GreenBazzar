...
public class PendingSellerProductsCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            productList = productService.findProductsByStatus(sellerId, String.valueOf(Status.PENDING));
            session.setAttribute(RequestParameter.USER_ID, sellerId);
            request.setAttribute(RequestParameter.PRODUCTS, productList);
            return new Router(PagePath.PENDING_SELLER_PRODUCTS_PAGE, Router.Type.FORWARD);
